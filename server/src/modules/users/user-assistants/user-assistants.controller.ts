...
export class UserAssistantsController {
  ...
  @Post()
  async create(@UserId() userId: string, @Body() assistantData: CreateAssistantDto): Promise<Assistant> {
    return this.assistantsService.createAssistant(userId, assistantData)
  }

  @ApiOperation({ summary: '删除用户助手' })
  @Delete(':aid')
  async delete(@UserId() userId: string, @Param('aid', ParseIntPipe) assistantId: number): Promise<void> {
  ...
  async update(
    ...
    return this.assistantsService.updateAssistant(userId, assistantId, assistantData)
  }
}
