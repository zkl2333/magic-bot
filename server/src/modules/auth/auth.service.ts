...
export class AuthService {
  ...
  async register(body: RegisterDto) {
    ...
    // 验证邮箱验证码
    const emailCodeExists = await this.prismaService.verificationCode.findUnique({
      where: {
        contact: email
      }
    })
    ...
    // 验证用户名是否已经注册
    const usernameExists = await this.usersService.checkUsernameExists(body.username)
    if (usernameExists) {
